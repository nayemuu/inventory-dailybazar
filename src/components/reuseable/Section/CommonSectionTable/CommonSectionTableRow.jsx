...
function CommonSectionTableRow(props) {
  const {
    item,
    fieldToShow,
    index,
  ...
  const handleAlert = (index, key) => {
    if (item.id) {
      console.log(
        `In line no ${index + 1}, Row with ${
          item.id
        } Id is Missing "${key}" field`
      );
    } else {
      console.log(`In line no ${index + 1}, "${key}" field is Missing`);
    }
    return 'missing';
  };
...
export default CommonSectionTableRow;
